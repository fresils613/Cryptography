...
class Encryption:    
    def file_encryption(self):
        ...
        cprint('Do you want to encrypt the key with public key?? true or false: ',end=' ', color='blue', attrs=['bold','blink'])
        self.public_key = input()
        if self.public_key.lower() == 'true':
            ...
            [encrypted_file_object.write(x) for x in (cipher_aes.nonce, tag, ciphertext)]

        elif self.public_key.lower() == 'false':
            encrypted_security_object.write(AES_key) # Storing our cipher text in a file           
            [encrypted_file_object.write(x) for x in (cipher_aes.nonce, tag, ciphertext)]
        else:
    ...
    def folder_encryption(self):
        ...
        for new_path in sorted(Path(self.Directory).iterdir(), key=lambda p: str(p).lower()):
            with open(new_path, 'rb') as p:
                ...
                textFile = p.read() # Reading the Plaintext File
